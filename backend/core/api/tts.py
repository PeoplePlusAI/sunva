...
async def tts_websocket(
    ...
    try:
        ...
        while True:
            ...
            selected_language = message.get("language", "en") # Set language everywhere for pipeline.
            ...
            if "text" in message:
                ...
                wav_data = await asyncio.get_event_loop().run_in_executor(
                    executor, text_to_speech, enhanced_text, tts_model, selected_language
                )
            else:
                wav_data = None
            ...
            # Try to send the response, but handle WebSocket disconnection gracefully
            try:
        ...
        # Handle final cleanup and cache persistence
